...
void Tet_rotat_Shap(u8 id,s8 x_center , s8 y_center , u16 Color)
{
	u8 tmp;
	tmp=figures[id][0];
...
void Tet_DrawSqr(u8 x , u8 y , u16 Color)
{
...
void DrawShap(u8 id , s8 x_center , s8 y_center , u16 Color)
{
	...
	for(u8 r = 0 ; r<3 ; r++)
	{
		...
		for(u8 c = 0 ; c<3 ; c++)
		{
			...
			if(figures[id][i]==1)
			{
				Tet_DrawSqr(x_center+X_Point,y_center+Y_Point,Color);
			}
			i++;
			X_Point++;
		}
		...
		for(u8 r = 0 ; r<3 ; r++)
		{
			...
			for(u8 c = 0 ; c<3 ; c++)
			{
				...
				if(figures[id][i]==1)
					...
					{
						Tet_DrawSqr(x_center+X_Point,y_center+Y_Point,0x0019);
					}
					else
					{
						Tet_DrawSqr(x_center+X_Point,y_center+Y_Point,0x0006);
					}
							}
				i++;
				X_Point++;
...
void Move(void)
{
	...
	case Dir_up:
		//rotate
		Tet_rotat_Shap(shap_currant[count],X_move , Y_move,color_shap_cur[count]);
		Tet_Dir=0;
		break;
	case Dir_left:
		//shift Left
